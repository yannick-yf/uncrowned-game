...
| Q40 | **Greyhold is a power base with no location.** §3 names it one of the six and §4's eight zones do not include it, so none of its levers — turn the lord, blackmail him, discredit him — can be built, and the sub-castle the regional law is administered from does not exist on the map. The other five power bases give every tracked quantity an input, so this blocks nothing; it leaves §3 naming a place the player cannot go | §3, §4 | Route B, and §3's sixth lever |
