...
func build_housing(builder: SceneTree) -> void:
	...
	shell(Vector3(-.55,0,-.30),3.6,3.5,2.65,"stone","hip",0,1.13)
	...
	start("baraquement_des_equipes","Baraquement des équipes","Low timber barrack with two entrances and end chimneys.")
	...
	shell(Vector3.ZERO,3.35,3.85,3.52,"half_timber","hip",0,1.12)
	...
	start("baraquement_a_galerie","Baraquement à galerie","Stone lower floor and timber upper floor with a left-hand stair.")
	...
	start("logis_du_contremaitre_en_l","Logis du contremaître en L","L-shaped masonry residence with a taller front room.")
	...
	start("maison_des_charretiers","Maison basse des charretiers","Wide low gable with a small rear storage projection.")
...
func build_industry(builder: SceneTree) -> void:
	b=builder
	start("tri_minerai_a_pignon","Halle de tri à pignon","Open transverse gable with three low sorting bays.","stockage")
	open_hall(6.5,3.85,2.73,"gable",90,1.15)
	for i: int in 3:b._bin(Vector3(-2.16+i*2.16,0,-.42),1.72,1.7,"slag" if i==2 else "ore")
	finish("transverse_gabled_sorting_hall","industrial")
	start("charbon_halle_a_croupes","Halle à charbon à croupes","Hipped canopy and low ventilated charcoal bays.","stockage")
	open_hall(5.85,4.15,2.65,"hip",90,1.12)
	b._bin(Vector3(-1.35,0,-.53),2.0,2.0,"charcoal")
	b._bin(Vector3(1.25,0,-.53),1.9,2.0,"charcoal")
	b._sacks(Vector3(1.60,0,1.17));finish("hipped_ventilated_charcoal_hall","industrial")
	start("forge_de_finition_a_pignon","Forge de finition à pignon","Gabled finishing forge with an exposed truss and open work face.","production")
	open_hall(6.0,4.7,2.90,"gable",0,1.48)
	b._hearth(Vector3(-1.45,0,-1.20));b._anvil(Vector3(.1,0,.45));b._bellows(Vector3(-.18,.24,-1.20),PI*.5)
	b._trough(Vector3(1.9,0,-.4),1.2,.7);b._tool_rack(Vector3(1.8,0,-2.08))
	finish("gabled_open_finishing_forge","industrial")
	start("reserve_barres_longue","Réserve de barres en longueur","Narrow deep gabled warehouse with two long storage racks.","stockage")
	open_hall(4.55,5.5,2.62,"gable",0,1.25)
	for x: float in [-1.28,1.28]:
		b._bars(Vector3(x,0,-.3),3.15,4)
		for side: float in [-.50,.50]:
			for z: float in [-1.7,1.2]:b._beam("RackPosts",Vector3(x+side,0,z),Vector3(x+side,1.43,z),.13)
		b._box("RackShelf",Vector3(1.1,.12,3.15),Vector3(x,1.07,-.22),"wood",Vector3.ZERO,true)
		b._bars(Vector3(x,1.13,-.22),2.85,2)
	finish("narrow_deep_gabled_bar_store","industrial")
	start("remise_des_betes_de_trait","Remise des bêtes de trait","Hipped stable with two stalls and a closed fodder room.","village")
	open_hall(6.1,4.15,2.66,"hip",90,1.28)
	for x: float in [-2.05,0.0]:
		b._box("Manger",Vector3(1.45,.52,.65),Vector3(x,.4,-1.60),"wood",Vector3.ZERO,true)
		for side: float in [-.5,.5]:b._box("MangerFeet",Vector3(.14,.22,.45),Vector3(x+side,.11,-1.60),"wood")
		b._hay(Vector3(x,.66,-1.60),1.20,.42)
	b._boarding(Vector3(-1.02,0,-.40),2.8,1.25,PI*.5)
	shell(Vector3(2.02,0,-.37),1.70,3.1,2.18,"timber","lean",0,.36)
	door(Vector3(2.02,0,1.18),.80,1.73)
	finish("hipped_two_stall_stable_fodder_room","service")
	start("latrines_a_deux_places","Latrines à deux places","Two small compartments under a transverse gable.","village")
	shell(Vector3.ZERO,2.65,1.80,1.93,"timber","gable",90,.67)
	for x: float in [-.66,.66]:door(Vector3(x,0,.90),.66,1.55)
	finish("double_compartment_latrine","service")

func start(id: String,label: String,description: String,family: String="village") -> void:
	b._start(id,family,label,description,"Bâtiment unique de l’aciérie ; conserver son accès et sa fondation.")
...
func shell(at: Vector3,width: float,depth: float,height: float,wall: String,roof: String,axis: float=0,rise: float=1.2) -> void:
	...
	b._box("SolidWalls",Vector3(width,height-.22,depth),at+Vector3(0,(height+.22)*.5,0),"mortar" if wall=="stone" else "dark_wood",Vector3.ZERO,true)
	var lower: float=height if wall=="stone" else (1.10 if wall=="half_timber" else .52)
	for side: float in [-1.0,1.0]:
		b._masonry_face(at+Vector3(0,0,side*(depth*.5+.015)),width,lower,0 if side>0 else PI)
		b._masonry_face(at+Vector3(side*(width*.5+.015),0,0),depth,lower,side*PI*.5)
	if wall!="stone":
		for side: float in [-1.0,1.0]:
			if wall=="timber":
				b._boarding(at+Vector3(0,lower,side*depth*.5),width,height-lower)
				b._boarding(at+Vector3(side*width*.5,lower,0),depth,height-lower,PI*.5)
			else:
				b._box("UpperPlaster",Vector3(width,height-lower,.12),at+Vector3(0,(height+lower)*.5,side*depth*.5),"pale_plaster")
				b._box("UpperPlaster",Vector3(.12,height-lower,depth),at+Vector3(side*width*.5,(height+lower)*.5,0),"pale_plaster")
				b._beam("UpperWallPlate",at+Vector3(-width*.5,height,side*(depth*.5+.08)),at+Vector3(width*.5,height,side*(depth*.5+.08)),.14,"dark_wood")
				for x: float in [-width*.5,width*.5]:b._beam("HalfTimberPosts",at+Vector3(x,lower,side*(depth*.5+.08)),at+Vector3(x,height,side*(depth*.5+.08)),.13,"dark_wood")
				b._beam("SideFrameBrace",at+Vector3(side*(width*.5+.08),lower,-depth*.43),at+Vector3(side*(width*.5+.08),height,depth*.43),.12,"dark_wood")
...
func roof_on(at: Vector3,width: float,depth: float,eave: float,roof: String,axis: float,rise: float,closed: bool,wall: String="timber") -> void:
	...
	if roof=="lean":
		...
		b._lean_roof(Vector3.ZERO,w,d,eave,rise,"rust_roof")
		...
		if roof=="offset":
			...
			roof_face(Vector3(w*.5+.18,eave,d*.5+.18),Vector3(w*.5+.18,eave,-d*.5-.18),Vector3(offset,eave+rise,d*.5+.18),Vector3(offset,eave+rise,-d*.5-.18))
			b._beam("OffsetRidge",Vector3(offset,eave+rise+.06,-d*.5-.2),Vector3(offset,eave+rise+.06,d*.5+.2),.14,"stone")
		else:b._roof(Vector3.ZERO,w,d,eave,rise,"roof")
	b.model_transform=saved

func gable_infill(width: float,depth: float,eave: float,rise: float,offset: float,mat: String) -> void:
	var st: SurfaceTool=b._surface("ClosedGable",mat)
	for side: float in [-1.0,1.0]:
		var a: Vector3=Vector3(-width*.5,eave,side*depth*.5)
		var c: Vector3=Vector3(width*.5,eave,side*depth*.5)
		var top: Vector3=Vector3(offset,eave+rise,side*depth*.5)
		b._tri(st,a,top,c)
		if mat=="dark_wood":
			b._beam("GableFrame",a,top,.10,"wood");b._beam("GableFrame",top,c,.10,"wood")
			b._beam("GableKingPost",Vector3(offset,eave,side*depth*.5),top,.10,"wood")
...
func hip_roof(width: float,depth: float,eave: float,rise: float) -> void:
	...
	for pair: Array in [[a,p],[c,p],[d,q],[e,q],[p,q]]:b._beam("HipCaps",pair[0]+Vector3.UP*.07,pair[1]+Vector3.UP*.07,.13,"stone")
...
func roof_face(a: Vector3,c: Vector3,p: Vector3,q: Vector3) -> void:
	...
	var tiles: SurfaceTool=b._surface("HipSlate","roof")
...
func canopy(at: Vector3,width: float,depth: float,height: float,rise: float) -> void:
	...
	b._lean_roof(at,width,depth,height,rise,"rust_roof")
