...
@export_range(0.01,0.15,0.001) var surface_offset_m: float = 0.04
var _ground: Node3D
func _ready() -> void:
...
func _refresh() -> void:
	...
	for i: int in range(samples.size()-1):
		...
		var bands: int=maxi(4,ceili(width_m/.45))
		for band: int in bands:
			var left:float=-1.0+2.0*float(band)/bands; var right:float=-1.0+2.0*float(band+1)/bands
			...
			for j: int in positions.size():
				...
				at.y=_ground.call("surface_height_at_world",at.x,at.z)+surface_offset_m
