extends Node3D
## Inspection UI for the sawmill kit. It does not modify the world map or assets.
var catalog: Array = []
var selected: int = 1
...
func _ready() -> void:
	catalog = JSON.parse_string(FileAccess.get_file_as_string("res://assets/sawmill/catalog.json")).assets
	_build_ui()
	show_demo()
...
func _build_ui() -> void:
	...
	var heading: Label=Label.new();heading.text="SCIERIE  /  BIBLIOTHÈQUE";heading.add_theme_font_size_override("font_size",19);column.add_child(heading)
	var count: Label=Label.new();count.text=str(catalog.size())+" assets · colombages, pierre et tuiles · échelle en mètres";count.add_theme_font_size_override("font_size",12);column.add_child(count)
...
func show_demo() -> void:
	...
	focus=Vector3(0,1.8,5);yaw=24;tilt=43;zoom=47
	title_label.text="Le village de la scierie"
	description.text="Grumes → sciage → séchage → charpente → chargement.\nAssemblage de présentation ; bâtiments à placer ensuite dans la carte."
